...
-record(backend_tracking, {
  ...
  max_failure_threshold = dcos_l4lb_config:tcp_consecutive_failure_threshold(),
  last_failure_time = 0,
  failure_backoff =  dcos_l4lb_config:tcp_failed_backend_backoff_period() * 1.0e6
  }).
...
-record(backend, {
  ip_port :: ip_port(),
  clock = fun dcos_l4lb_ewma:now/0,
