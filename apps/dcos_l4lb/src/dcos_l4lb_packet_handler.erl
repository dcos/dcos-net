...
%%%-------------------------------------------------------------------
-module(dcos_l4lb_packet_handler).
-author("sdhillon").
-author("Tyler Neely").
...
handle(NFQPid, Info) ->
  ...
  supervisor:start_child(dcos_l4lb_worker_sup, ChildSpec).
...
do_handle(NFQPid, Info) ->
  ...
  dcos_l4lb_metrics:update([mapping_time_us], Now2-Now1, histogram),
  case MappingRet of
    {ok, Mapping} ->
      lager:debug("Mapping: ~p", [Mapping]),
      dcos_l4lb_ct:install_mapping(Mapping);
    Else ->
  ...
  dcos_l4lb_metrics:update([packet_handler_total_time_us], Now3-Now1, histogram).


get_src_addr(SrcAddr, BackendIP) ->
  case {dcos_l4lb_iface_server:is_local(SrcAddr), dcos_l4lb_iface_server:is_local(BackendIP)} of
    {true, true} ->
      {127, 0, 0, 1};
    {false, true} ->
      SrcAddr;
    {_, false} ->
      {ok, Route} = dcos_l4lb_routes:get_route(BackendIP),
...
to_mapping(Payload) ->
  ...
  case dcos_l4lb_vip_server:get_backend(DstAddr, DstPort) of
...
foreign_to_foreign_test() ->
  ...
  {ok, _} = dcos_l4lb_vip_server:start_link_nosubscribe(),
  VIP = {tcp, {1, 1, 1, 1}, 1000},
  Backend = {{8, 8, 8, 8}, 31421},
  VIPs = orddict:store(VIP, [Backend], []),
  dcos_l4lb_vip_server:push_vips(VIPs),
