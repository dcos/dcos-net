...
init([]) ->
  ok = timer:start(),
  {ok, _} = timer:send_after(dcos_l4lb_config:poll_interval(), poll),
  %% 64MB
...
prune_old_agents(AgentCache) ->
  map:filter(fun (#agent_cache{last_seen_time = LastSeenTime}) ->
                   ...
                   Now - LastSeenTime =< dcos_l4lb_config:agent_reregistration_threshold()
               end, AgentCache).
...
parse_json_to_vips(State = #state{last_master_elected = LastMaster,
  ...
  LastMasterCanKillOldAgents = Now - LastMasterTime > dcos_l4lb_config:agent_reregistration_threshold(),
