...
install_mapping(Mapping) ->
  ...
  dcos_l4lb_metrics:update([ct_install_time_us, WorkerID], Now2-Now1, histogram),
  dcos_l4lb_metrics:update([ct_install_time_us], Now2-Now1, histogram),
  Ret.
%%--------------------------------------------------------------------
%% @doc
%% Starts the server
...
try_mapping(Mapping, Socket) ->
  ...
  case Status of
    ok ->
      dcos_l4lb_metrics:update([ct_success], 1, spiral),
      ok;
    {error, Error} ->
      dcos_l4lb_metrics:update([ct_failure], 1, spiral),
