...
load_rule({Table, Chain, Rule}) ->
  Rule1 = re:replace(Rule, "QUEUE_NUMBER", integer_to_list(dcos_l4lb_config:queue()),  [global, {return, list}]),
...
handle_packet(Info) ->
  ...
  dcos_l4lb_packet_handler:handle(Payload).