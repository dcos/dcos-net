...
handle_remove_service(Service, #state{netlink_generic = Pid, family = Family}) ->
    case dcos_l4lb_netlink:request(Pid, Family, ipvs, [], #del_service{request = [{service, Service}]}) of
        {ok, _} -> ok;
        _ -> error
    end.
...
handle_add_service(IP, Port, #state{netlink_generic = Pid, family = Family}) ->
    ...
    case dcos_l4lb_netlink:request(Pid, Family, ipvs, [], #new_service{request = [{service, Service1}]}) of
        {ok, _} -> ok;
        _ -> error
    end.
...
handle_add_dest(Service, IP, Port, #state{netlink_generic = Pid, family = Family}) ->
    ...
    case dcos_l4lb_netlink:request(Pid, Family, ipvs, [], Msg) of
        {ok, _} -> ok;
        _ -> error
    end.
