...
init([]) ->
    ...
    {ok, IFaceMgr} = dcos_l4lb_iface_mgr:start_link(?MINUTEMAN_IFACE),
...
do_reconcile_iface(VIPs, #state{iface_mgr = IFaceMgr}) ->
    IFaceIPs = ordsets:from_list(dcos_l4lb_iface_mgr:get_ips(IFaceMgr)),
    ExpectedIPs = ordsets:from_list([VIP || {{_Proto, VIP, _Port}, _Backends} <- VIPs]),
    IPsToAdd = ordsets:subtract(ExpectedIPs, IFaceIPs),
    lists:foreach(fun(IP) -> ok = dcos_l4lb_iface_mgr:add_ip(IFaceMgr, IP) end, IPsToAdd),
    IPsToRemove = ordsets:subtract(IFaceIPs, ExpectedIPs),
    lists:foreach(fun(IP) -> ok = dcos_l4lb_iface_mgr:remove_ip(IFaceMgr, IP) end, IPsToRemove).
...
modify_service({_Protocol, IP, Port}, BEAdd, BERemove, #state{ipvs_mgr = IPVSMgr}) ->
    lists:foreach(
        fun({BEIP, BEPort}) ->
            dcos_l4lb_ipvs_mgr:add_dest(IPVSMgr, IP, Port, BEIP, BEPort)
        end,
        BEAdd),
    lists:foreach(
        fun({BEIP, BEPort}) ->
            dcos_l4lb_ipvs_mgr:remove_dest(IPVSMgr, IP, Port, BEIP, BEPort)
        end,
        BERemove).


remove_service({_Protocol, IP, Port}, #state{ipvs_mgr = IPVSMgr}) ->
    dcos_l4lb_ipvs_mgr:remove_service(IPVSMgr, IP, Port).

add_service({_Protocol, IP, Port}, BEs, #state{ipvs_mgr = IPVSMgr}) ->
    dcos_l4lb_ipvs_mgr:add_service(IPVSMgr, IP, Port),
    lists:foreach(fun({BEIP, BEPort}) ->  dcos_l4lb_ipvs_mgr:add_dest(IPVSMgr, IP, Port, BEIP, BEPort) end, BEs).
