...
pick_backend_internal(BackendAddrs) when length(BackendAddrs) > ?BACKEND_LIMIT ->
  dcos_l4lb_metrics:update([probabilistic_backend_picker], 1, spiral),
  Now = erlang:monotonic_time(),
...
pick_backend_internal(BackendAddrs) ->
  dcos_l4lb_metrics:update([simple_backend_picker], 1, spiral),
...
probabilistic_backend_chooser(_Now, _ReachabilityCache, _RemainingBackendAddrs, ReachableAndOpenBackends,
  ...
  dcos_l4lb_metrics:update([reachable_open_backend], 1, spiral),
...
make_backend_choice(_ReachableAndOpenBackends, [OpenBackend1, OpenBackend2|_], _OtherBackends) ->
  dcos_l4lb_metrics:update([open_backend], 1, spiral),
...
make_backend_choice(_ReachableAndOpenBackends, _OpenBackends, [OtherBackend]) ->
  dcos_l4lb_metrics:update([other_backend], 1, spiral),
  OtherBackend;
...
is_reachable_real(_IP, false) ->
  false;
is_reachable_real(IP, Tree) ->
  Nodenames = dcos_l4lb_lashup_index:nodenames(IP),
...
notify_metrics(true, {IP, Port}) ->
  dcos_l4lb_metrics:update([backend, {IP, Port}, successes], 1, spiral);
notify_metrics(false, {IP, Port}) ->
  dcos_l4lb_metrics:update([backend, {IP, Port}, failures], 1, spiral);
