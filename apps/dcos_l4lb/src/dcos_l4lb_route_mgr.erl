...
init([]) ->
    {ok, Pid} = dcos_l4lb_netlink:start_link(?NETLINK_ROUTE),
...
get_routes(Pid) ->
    {ok, Raw} = dcos_l4lb_netlink:rtnl_request(Pid, getroute, [match, root], {inet, 0, 0, 0, 0, 0, 0, 0, [], []}),
    Routes0 =
...
add_route(Dst, #state{netlink = Pid}) ->
    ...
    Route = {
        ...
        _Flags = [],
        Msg},
    {ok, _} = dcos_l4lb_netlink:rtnl_request(Pid, newroute, [create, excl], Route).
...
remove_route(Dst, #state{netlink = Pid}) ->
    ...
    Route = {
        ...
        _Flags = [],
        Msg},
    {ok, _} = dcos_l4lb_netlink:rtnl_request(Pid, delroute, [], Route).

ensure_fib_rule(Pid) ->
    {ok, Rules} = dcos_l4lb_netlink:rtnl_request(Pid, getrule, [match, root], {inet, 0, 0, 0, 0, 0, 0, 0, [], []}),
    ...
    case Match of
        [] ->
            ...
            dcos_l4lb_netlink:rtnl_request(Pid, newrule, [create, excl], Rule);
        _ ->
            ok
    end.
