...
metrics_for_path("/vip/" ++ Vip) ->
  case parse_ip_port(Vip) of
    {IP, Port} ->
      case dcos_l4lb_vip_server:get_backends_for_vip(IP, Port) of
        {ok, Backends} ->
...
vip_metrics() ->
  ...
  % get stats for each
  LiveVips = dcos_l4lb_vip_server:get_vips(),
...
metrics_for_backend({IP, Port}) ->
  Backend = #backend{tracking = Tracking} = dcos_l4lb_ewma:get_ewma({IP, Port}),
  Cost = dcos_l4lb_ewma:cost(Backend),
  Healthy = dcos_l4lb_ewma:is_open(Tracking),
