...
test_ip_cmds_3(Config) ->
    ...
    Family = inet,
    PrefixLen = 32,
    {ok, _} = dcos_overlay_netlink:ipaddr_replace(Pid, Family, IP, PrefixLen, ?IFNAME),
...
test_ip_cmds_4(Config) ->
    ...
    MacStr = mac_ntoa(Mac),
    Family = inet,
    {ok, _} = dcos_overlay_netlink:ipneigh_replace(Pid, Family, IP, Mac, ?IFNAME),
...
test_iprule_cmds(Config) ->
    ...
    SrcPrefixLen = 8,
    Family = inet,
    {ok, _} = dcos_overlay_netlink:iprule_add(Pid, Family, Src, SrcPrefixLen, 42),
    ...
    %% Test2: ip rule show
    {ok, Rules} = dcos_overlay_netlink:iprule_show(Pid, Family),
    Rule = dcos_overlay_netlink:make_iprule(Family, Src, SrcPrefixLen, 42),
