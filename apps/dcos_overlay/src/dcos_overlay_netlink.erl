...
-export([
    start_link/0, stop/1, init_metrics/0, ipneigh_replace/5, iproute_replace/6,
    bridge_fdb_replace/4, iplink_show/2, iplink_add/6, iplink_set/3,
    iprule_show/2, iprule_add/5, make_iprule/4, match_iprules/2,
    is_iprule_present/2, iplink_delete/2, ipaddr_replace/5, if_nametoindex/1]).
...
%% eg. ipneigh_replace(Pid, inet, {44,128,0,1},
%%         {16#70,16#b3,16#d5,16#80,16#00,16#03}, "vtep1024").
ipneigh_replace(Pid, Family, Dst, Lladdr, Ifname) ->
    Attr = [{dst, Dst}, {lladdr, Lladdr}],
    case if_nametoindex(Ifname) of
        {ok, Ifindex} ->
            Neigh = {
                _Family = Family,
                _Ifindex = Ifindex,
...
%% eg. bridge_fdb_replace(Pid, {192,168,65,91},
%%         {16#70,16#b3,16#d5,16#80,16#00,16#03}, "vtep1024").
...
iplink_add(Pid, Ifname, Kind, Id, DstPort, Attr) ->
    Vxlan = [
        {id, Id}, {ttl, 0}, {tos, 0}, {learning, 1}, {proxy, 0},
...
make_iprule(Family, Src, SrcPrefixLen, Table) ->
    {Family, 0, SrcPrefixLen, 0, Table, unspec, universe, unicast, [],
     [{src, Src}]}.
...
netlink_request(Pid, Type, Flags, Msg) ->
    case gen_netlink_client:rtnl_request(Pid, Type, Flags, Msg) of
        {error, Code, Message} ->
            prometheus_counter:inc(overlay, netlink_errors_total, [], 1),
            %% just to make error handling in callers a bit more uniform
            {error, {Code, Message}};
        Result ->
            Result
    end.
...
if_nametoindex(Ifname) ->
    case gen_netlink_client:if_nametoindex(Ifname) of
        {error, Error} ->
            prometheus_counter:inc(overlay, netlink_errors_total, [], 1),
            {error, Error};
        Result ->
